...
# feedback_rows <- complete_data %>%
#   filter(measure == "feedback")


# quickness_rows <- complete_data %>%
...
#   ungroup()
# # so there are only 19 people who actually answered every question
...
# Step 2: Filter the original dataset to keep only rows with those IDs
filtered_data <- complete_data %>%
  filter(ID %in% unique_ids$ID)


distinct_id_count <- filtered_data %>%
...
exclu_data <- filtered_data %>%
  ...
  filter(any(measure == "attention" & grepl("nate|justin", response, ignore.case = TRUE))) %>%
   # 38 left after excluding those that failed the attention check
...
ggplot(character_data, 
  ...
  labs(title = "Character Judgements by   \n Morality Condition and Decision Speed",
       ...
       fill = "Decision Speed") +
  scale_fill_manual(values = c("f" = "lightskyblue", "s" = "maroon1"), #Custom colors
...
lineplot <- ggplot(character_data, 
       aes(x = condition, y = response, color = FastorSlow, group = FastorSlow)) +
  stat_summary(fun = mean, # Compute mean of response
  ...
  scale_color_manual(values = c("f" = "lightskyblue", "s" = "maroon1"), # Custom colors
                     labels = c("f" = "Quick", "s" = "Slow")) +
  scale_y_continuous(breaks = 1:7) +
  coord_cartesian(ylim = c(1, 7)) +
  theme_bw() +
