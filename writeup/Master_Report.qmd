...
barplot <- ggplot(character_data, 
  ...
  labs(x = "Morality Condition",
       y = "(Positive) Moral Character Evaluation",
       fill = "Decision Speed") +
  scale_fill_manual(values = c("f" = "lightskyblue", "s" = "maroon1"),
                    labels = c("f" = "Quick", "s" = "Slow")) +
  scale_y_continuous(breaks = 1:7) +
  coord_cartesian(ylim = c(1, 7)) +
  theme_bw() +
...
lineplot <- ggplot(character_data, 
  ...
  scale_color_manual(values = c("f" = "lightskyblue", "s" = "maroon1"), 
                     labels = c("f" = "Quick", "s" = "Slow")) +
  scale_y_continuous(breaks = 1:7) +
  coord_cartesian(ylim = c(1, 7)) +
  theme_bw() +
...
# Fit a mixed model (accounts for ID) with the interaction of interest
lmemod <- lme(response_reverse ~ condition * FastorSlow, random = ~1|ID, 
               data = character_data)
...
```{r}
qqnorm(resid(intmod)); qqline(resid(intmod))
```
